...
class Canvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if (self.drawing() or self.continueDrawing()) and not self.prevPoint.isNull() and not self.outOfPixmap(self.prevPoint):
            ...
            p.drawLine(int(self.prevPoint.x()), int(0), int(self.prevPoint.x()), int(self.pixmap.height()))
            p.drawLine(int(0), int(self.prevPoint.y()), int(self.pixmap.width()), int(self.prevPoint.y()))
